...
Class Lib {
    ...
    /*public function getDbToken($url) {
        $str_STATUT = "enable";
        $sql = "SELECT * FROM j3_smstoken WHERE str_STATUT = :str_STATUT ORDER BY dt_CREATED DESC";
    ...
    public function addSMS($str_DESTINATAIRE, $str_DESCRIPTION, $db) {
        $arrayJson = array();
        $message = "";
        $code_statut = "";
        $str_STATUT = "enable";

        $sql = "INSERT INTO t_sms(lg_SMS_ID,str_DESTINATAIRE,str_DESCRIPTION,str_STATUT,dt_CREATED)"
                . "VALUES (:lg_SMS_ID,:str_DESTINATAIRE,:str_DESCRIPTION,:str_STATUT,:dt_CREATED)";
        try {
            ...
            switch ($int) {
                ...
                case 3:
                    if ($stmt->execute()) {
                        $success = $this->sendSMS($str_DESTINATAIRE, $str_DESCRIPTION, URL_SENDSMS);
                        //var_dump($success);
                        if ($success) {
    ...
    public function addSmstoken($str_ACCESSTOKEN, $str_EXPRIRESIN, $int_DAYSNUMBER, $db) {
        ...
        $sql = "INSERT INTO j3_smstoken(str_ACCESSTOKEN,int_EXPRIRESIN,int_DAYSNUMBER,dt_EXPIRATION,str_STATUT,dt_CREATED)"
...
//            $array_index = array('PARTNERCONTRACTS', 'CONTRACTS', '0', 'SERVICECONTRACTS');
    ...
    public function sendSMS($tel_number, $message, $url) {
...
//            print_r($json_decode);
    ...
    public function processDescativeToken() {
        ...
        try {
            ...
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($result as $item_result) {
                $sql_updated = "UPDATE j3_smstoken t "
